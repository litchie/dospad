...
static void HandleMouseMotion(SDL_MouseMotionEvent * motion) {
    if (sdl.mouse.locked || !sdl.mouse.autoenable) {
        if(motion->relative_mode) {
            ...
            // the event has been generated by the SDL mouse in non-relative mode
            // respect its x and y coordinate as they are
            Mouse_CursorSet(motion->x, motion->y);
        }
    }
}
...
#ifndef IPHONEOS  // Don't create config file
	if(!parsed_anyconfigfile) {
		//Try to create the userlevel configfile.
		config_file.clear();
		Cross::CreatePlatformConfigDir(config_path);
