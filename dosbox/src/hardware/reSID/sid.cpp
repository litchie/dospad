...
void SID2::SaveState( std::ostream& stream )
{
	...
	WRITE_POD( &clock_frequency, clock_frequency );
	WRITE_POD( &ext_in, ext_in );
	WRITE_POD( &bus_value, bus_value );

	WRITE_POD( &sampling, sampling );
	WRITE_POD( &cycles_per_sample, cycles_per_sample );
	WRITE_POD( &sample_offset, sample_offset );
	WRITE_POD( &sample_index, sample_index );
	WRITE_POD( &sample_prev, sample_prev );

	WRITE_POD( &fir_N, fir_N );
	WRITE_POD( &fir_RES, fir_RES );



	// - 'new' data
	WRITE_POD( &sample_ptr, sample_ptr);
	WRITE_POD( &fir_ptr, fir_ptr);

	if( sample_ptr == 1 ) WRITE_POD_SIZE( sample, sizeof(short) * (RINGSIZE * 2) );
	if( fir_ptr == 1 ) WRITE_POD_SIZE( fir, sizeof(short) * (fir_N * fir_RES) );



	for( int lcv=0; lcv<3; lcv++ )
		voice[lcv].SaveState(stream);

	filter.SaveState(stream);
	extfilt.SaveState(stream);
}
...
void SID2::LoadState( std::istream& stream )
{
	...
	READ_POD( &clock_frequency, clock_frequency );
	READ_POD( &ext_in, ext_in );
	READ_POD( &bus_value, bus_value );

	READ_POD( &sampling, sampling );
	READ_POD( &cycles_per_sample, cycles_per_sample );
	READ_POD( &sample_offset, sample_offset );
	READ_POD( &sample_index, sample_index );
	READ_POD( &sample_prev, sample_prev );

	READ_POD( &fir_N, fir_N );
	READ_POD( &fir_RES, fir_RES );



	// - 'new' data
	READ_POD( &sample_ptr, sample_ptr);
	READ_POD( &fir_ptr, fir_ptr);

	if( sample_ptr == 1 ) {
		if( sample == NULL ) sample = new short[RINGSIZE * 2];
		READ_POD_SIZE( sample, sizeof(short) * (RINGSIZE * 2) );
	}
	else if( sample ) delete[] sample;

	if( fir_ptr == 1 ) {
		if( fir == NULL ) sample = new short[fir_N * fir_RES];
		READ_POD_SIZE( fir, sizeof(short) * (fir_N * fir_RES) );
	}
	else if( fir ) delete[] fir;



	for( int lcv=0; lcv<3; lcv++ )
		voice[lcv].LoadState(stream);

	filter.LoadState(stream);
